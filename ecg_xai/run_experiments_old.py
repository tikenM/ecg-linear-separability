...
from .significance import paired_cv_comparison, pairwise_mcnemar
...
def run_mitdb(mitdb_dir: str, classifiers=("svc", "lr", "dt"),
    ...
    out["significance_mcnemar"] = pairwise_mcnemar(yte, test_preds)
...
def run_incart(incart_dir: str, classifiers=("svc", "lr", "dt")) -> dict:
    print("[INCART] loading + patient-grouped hold-out ...")
    beats = data_io.load_dataset(
        incart_dir, [f"I{n:02d}" for n in range(1, 76)], leads=("II", "V1"))
    train, test = data_io.patient_grouped_holdout(beats, test_frac=0.3)
    Xtr, names, family, band_map = _featurize(train)
    Xte, *_ = _featurize(test)
    out = {"n_train": len(train), "n_test": len(test), "interpatient": {}}
    for name in classifiers:
        pipe = build_pipeline(name)
